...
func NewAccrualStore() *AccrualStore {
	...
	number := 2377225624
	av := core.Answer{Number: number, Status: "PROCESSED", Accrual: 500}
	m[number] = av
...
func (m *AccrualStore) GetLoyaltyPointsByOrderNumber(number int, ctx context.Context) (core.Answer, error) {
	select {
	case <-ctx.Done():
	...
	default:
		...
		return core.Answer{Number: number, Status: "INVALID"}, nil
	}
}
